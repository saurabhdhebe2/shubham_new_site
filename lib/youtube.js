...
function parseRolesFromDescription(description) {
  if (!description) return DEFAULT_ROLE;
  const matches = description.match(/%([A-Za-z]+)/g) || [];
  if (matches.length === 0) return DEFAULT_ROLE;
  const seen = new Set();
  const roles = [];
  for (const raw of matches) {
    ...
    seen.add(key);
    roles.push(ROLE_TITLE_CASE[key] || (key.charAt(0).toUpperCase() + key.slice(1)));
  }
  return roles.join(' / ');
}
...
async function fetchPlaylist(playlistId, category) {
  ...
  try {
    ...
    return (data.items || []).map((item, i) => {
      const uploadedAt = item.contentDetails?.videoPublishedAt || item.snippet.publishedAt;
      return {
        ...
        year: new Date(uploadedAt).getFullYear(),
        role: parseRolesFromDescription(item.snippet.description),
        deliverables: '',
